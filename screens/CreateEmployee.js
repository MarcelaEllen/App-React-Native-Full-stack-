...
const CreateEmployee = ({ navigation, route }) => {
    ...
    if (route.params) {
      switch (type) {
        ...
        case 'projectExperience':
          return route.params.projectExperience;
      }
    }
    return '';
  };
  ...
  const [projectExperience, setProjectExperience] = useState(getDetails('projectExperience'));
  const [modal, setModal] = useState(false);
  const [enableshift, setEnableShift] = useState(false);

  const submitData = () => {
    fetch('http://10.0.0.135:3000/send-data', {
      method: 'post',
      headers: {
      ...
      body: JSON.stringify({
        ...
        picture,
        position,
        projectExperience, // Novo atributo
      }),
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const updateDetails = () => {
    fetch('http://10.0.0.135:3000/update', {
      method: 'post',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  return (
    ...
    >
      <View>
        ...
        />
        <TextInput
          label="Experiência em Projetos"
          style={styles.inputStyle}
          value={projectExperience}
          theme={theme}
          onFocus={() => setEnableShift(true)}
          mode="outlined"
          onChangeText={(text) => setProjectExperience(text)}
        />
        <Button
          style={styles.inputStyle}
